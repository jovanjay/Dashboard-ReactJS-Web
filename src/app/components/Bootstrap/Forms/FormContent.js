...
class FormContent extends React.Component {
    ...
    sampleFormContent = (data, hasTitle, headerTitle, horizontal) => {
        return (<Card style={{minHeight:`455px`}}>
            <Card.Header>
                ...
                <Form>
                    ...
                    {!horizontal && data.map((item, index) => (
                        <Form.Group key={index} controlId={item.id} className="mb-3">
                            <Form.Label>{item.label}</Form.Label><Form.Control type={item.type} placeholder={item.placeholder}/>
                        </Form.Group>
                    ))}

                    {horizontal && data.map((item, index) => (
                        <Form.Group as={Row} key={index} controlId={item.id} className="mb-3">
                            <Form.Label column sm="2">{item.label}</Form.Label><Col sm="10"><Form.Control type={item.type} placeholder={item.placeholder}/></Col>
                        </Form.Group>
                    ))}

                    {!horizontal && (
                        <ButtonToolbar>
                            <Button className="mr-2" variant="primary" size="sm" type="submit">Submit</Button>    
            ...
            subMenu : [
                {
                    label : 'Mixins',
                    component : appComponents.FORMS,
                    parent : appComponents.FORMS
                },
                {
                    label : 'Data Driven',
                    component : appComponents.COMPLEX_FORMS,
                    parent : appComponents.FORMS
                },
                {
                    label : 'Form Wizard',
                    component : appComponents.FORM_WIZARD,
                    parent : appComponents.FORMS
                }
            ]
        }
